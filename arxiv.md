...
subject to trace non-increase. A finite instrument is a finite family $(\Phi_o)_o$ whose sum is trace non-increasing; measurement probabilities are $\operatorname{Tr}(\Phi_o(\rho))$, and normalization gives the conditional post-measurement state. `InstrumentPower n D` compares finite instruments by the Loewner order on the summed Choi matrices selected by every Scott-open outcome test, antisymmetrizes that preorder, and takes its lower-set completion. This gives a complete continuous lattice with Scott-continuous, order-enriched pushforward satisfying identity and composition. It does **not** give the required quantum power model: plain lower-set joins fail local continuity in the map argument, event predicates are too weak for value-dependent quantum Kleisli congruence, and the uncountable family of compact scalar instruments is incompatible with $\omega\mathbf{QVA}$ finite separation. A viable replacement requires a countable rounded-ideal or Scott-closed completion together with weakest-precondition tests valued in CP maps.
