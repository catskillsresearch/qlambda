...
/-- Value completeness under a mixed ordinary/recursive function-frame
spine over leftover argument frames. -/
def ValueUnderFunFrameSpine {C : Type}
    (D₀ : QDomain.{0})
    (j₀ : IsContinuousLatticeProjection D₀.carrier
      (QuantumFunctor (QModel (TTExternalContinuationPower 2)) D₀.carrier))
    (realize : C → HSemanticValue D₀ j₀)
    (frames : List (FunFrame C))
    (args : List (Term (QubitPrimitive C) × RuntimeEnv C)) :
    Prop :=
  ∀ {s : ChannelConfig C} {arg : RuntimeValue C}
    {answer : HSemanticComp D₀ j₀},
    s.control = .value arg →
    s.stack = funFrameStack frames ++ argumentStack args →
    ChannelConfig.WellScoped s →
    ChannelConfigRel D₀ j₀ realize s answer →
    PresentedChannelConfigCompleteness D₀ j₀ realize s answer

theorem valueUnderFunFrameSpine_nil
    {C : Type}
    (D₀ : QDomain.{0})
    (j₀ : IsContinuousLatticeProjection D₀.carrier
      (QuantumFunctor (QModel (TTExternalContinuationPower 2)) D₀.carrier))
    (realize : C → HSemanticValue D₀ j₀) :
    ValueUnderFunFrameSpine D₀ j₀ realize [] [] := by
  intro s arg answer hc hs hscoped hrel
  have hs' : s.stack = [] := by simpa [funFrameStack, argumentStack] using hs
  exact
    { related := hrel
      complete :=
        terminal_presentedChannelTreeCompleteness D₀ j₀ realize
...
theorem pauliX_under_fun_frame_spine_of_value
    {C : Type}
    (D₀ : QDomain.{0})
    (j₀ : IsContinuousLatticeProjection D₀.carrier
      (QuantumFunctor (QModel (TTExternalContinuationPower 2)) D₀.carrier))
    ...
    (args : List (Term (QubitPrimitive C) × RuntimeEnv C))
    (hne : frames ≠ [])
    (hVal : ValueUnderFunFrameSpine D₀ j₀ realize frames args)
    {s : ChannelConfig C} {value : C} {answer : HSemanticComp D₀ j₀}
    (hc : s.control = .term (.prim (.pauliX value)))
    (hs : s.stack = funFrameStack frames ++ argumentStack args)
    (hscoped : ChannelConfig.WellScoped s)
    (hrel : ChannelConfigRel D₀ j₀ realize s answer) :
    PresentedChannelConfigCompleteness D₀ j₀ realize s answer := by
  have hsPx :
  ...
  have hneStack : s.stack ≠ [] := by
    rw [hs]
    match frames, hne with
    | [], hne => exact False.elim (hne rfl)
    | f :: rest, _ =>
        simp [funFrameStack_ne_nil_of_cons f rest]
  let unitVal : HSemanticComp D₀ j₀ :=
    semanticUnit (Q := TTExternalContinuationPower 2)
      (D₀ := D₀) (j₀ := j₀) (realize value)
  let opVal : HSemanticComp D₀ j₀ :=
...
theorem admin_noapp_under_fun_frame_spine_of_value
    {C : Type}
    (D₀ : QDomain.{0})
    (j₀ : IsContinuousLatticeProjection D₀.carrier
      (QuantumFunctor (QModel (TTExternalContinuationPower 2)) D₀.carrier))
  ...
  | prim prim =>
      ...
      | pauliX value =>
          exact
            pauliX_under_fun_frame_spine_of_value
              D₀ j₀ realize frames args hne hVal hc hs hscoped hrel
      | measureZ _ _ =>
          exact False.elim hadmin

/-- Presented completeness for FunAppFrag under a mixed ordinary /
recursive function-frame spine over leftover argument frames. -/
theorem funAppFrag_under_fun_frame_spine_of_value
    {C : Type}
    (D₀ : QDomain.{0})
    (j₀ : IsContinuousLatticeProjection D₀.carrier
      (QuantumFunctor (QModel (TTExternalContinuationPower 2)) D₀.carrier))
    ...
    (hrel : ChannelConfigRel D₀ j₀ realize s answer) :
    PresentedChannelConfigCompleteness D₀ j₀ realize s answer := by
  revert frames args s answer hempty hVal hc hs hscoped hrel
  ...
  | admin hadmin =>
      intro
        ...
        (frames : List (FunFrame C))
        (args : List (Term (QubitPrimitive C) × RuntimeEnv C))
        hempty hVal hc hs hscoped hrel
      if hframes : frames = [] then
        have hargs : args = [] := hempty hframes
        subst frames
        subst args
        obtain ⟨semanticEnv, k, henv, hstack, heq⟩ :=
          channelConfigRel_term_inv D₀ j₀ hc hrel
        have hs' : s.stack = [] := by
          simpa [funFrameStack, argumentStack] using hs
        rw [hs'] at hstack
        cases hstack
        exact
          { related := hrel
        ...
        exact admin_noapp_under_fun_frame_spine_of_value
          D₀ j₀ realize frames args hframes hVal hadmin
          hc hs hscoped hrel
  | @app_lam x body arg hbody harg ihBody ihArg =>
      ...
      have hsArg :
          sArg.stack =
            funFrameStack (.clo x body s.env :: frames) ++
              argumentStack args := by
        simp [sArg, hs, funFrameStack]
      have hemptyNew :
          (.clo x body s.env :: frames) = [] → args = [] := by
        intro h; cases h
      have hValNew : ValueUnderFunFrameSpine D₀ j₀ realize
          (.clo x body s.env :: frames) args := by
        intro s' arg' answer' hc' hs' hscoped' hrel'
        have hsEq :
            {s' with
          ...
          have happ :
              ChannelInternalStep
                {s' with
                  ...
                  stack :=
                    .function (.closure x body s.env) ::
                      (funFrameStack frames ++ argumentStack args)}
                {s' with
                  control := .term body
                  env := RuntimeEnv.bind x arg' s.env
                  stack := funFrameStack frames ++ argumentStack args} :=
          ...
          ihBody
            (s :=
              {s' with
                ...
                env := RuntimeEnv.bind x arg' s.env
                stack := funFrameStack frames ++ argumentStack args})
            (answer := answer') frames args hempty hVal rfl rfl
            hscopedBody hrelBody
        exact beta_presentedChannelConfigCompleteness D₀ j₀ realize
          hc' (by simpa [funFrameStack] using hs') hrel' hchild
      have hargComplete :
          PresentedChannelConfigCompleteness D₀ j₀ realize sArg
            answer :=
        ihArg (s := sArg) (answer := answer)
          (.clo x body s.env :: frames) args hemptyNew hValNew
          rfl hsArg hscopedArg
          (by
  ...
  | @app_recLam self x body arg hbody harg ihBody ihArg =>
      intro
        (s : ChannelConfig C) (answer : HSemanticComp D₀ j₀)
        (frames : List (FunFrame C))
        (args : List (Term (QubitPrimitive C) × RuntimeEnv C))
      ...
      have hstepApp : ChannelInternalStep s
        ...
        have happ :
            ChannelInternalStep
              {s with
                control := .term (.app (.recLam self x body) arg)}
              {s with
                control := .term (.recLam self x body)
                stack := .argument arg s.env :: s.stack} :=
          ChannelInternalStep.application (s := s)
      ...
      have hsArg :
          sArg.stack =
            funFrameStack (.recClo self x body s.env :: frames) ++
              argumentStack args := by
        simp [sArg, hs, funFrameStack]
      have hemptyNew :
          (.recClo self x body s.env :: frames) = [] → args = [] := by
        intro h; cases h
      have hValNew : ValueUnderFunFrameSpine D₀ j₀ realize
          (.recClo self x body s.env :: frames) args := by
        intro s' arg' answer' hc' hs' hscoped' hrel'
        have hsEq :
          ...
          ChannelConfig.ext hc'.symm rfl hs'.symm rfl
        have hrelBody :=
          channel_config_recBeta D₀ j₀ (s := s') (self := self)
            (x := x) (body := body) (closureEnv := s.env)
            (arg := arg')
            (rest := funFrameStack frames ++ argumentStack args)
            (hsEq.symm ▸ hrel')
        have hstepBeta : ChannelInternalStep s'
              ...
              env :=
                RuntimeEnv.bind x arg'
                  (RuntimeEnv.bind self
                    (.recClosure self x body s.env) s.env)
              stack := funFrameStack frames ++ argumentStack args} := by
          have happ :
              ChannelInternalStep
                {s' with
                  ...
                  env :=
                    RuntimeEnv.bind x arg'
                      (RuntimeEnv.bind self
                        (.recClosure self x body s.env) s.env)
                  stack := funFrameStack frames ++ argumentStack args} :=
            ChannelInternalStep.recBeta (s := s') (self := self)
              (x := x) (body := body) (closureEnv := s.env)
              (arg := arg')
              (rest := funFrameStack frames ++ argumentStack args)
          exact hsEq.symm ▸ happ
      ...
      have hargComplete :
          PresentedChannelConfigCompleteness D₀ j₀ realize sArg
            answer :=
        ihArg (s := sArg) (answer := answer)
          (.recClo self x body s.env :: frames) args hemptyNew hValNew
          rfl hsArg hscopedArg
          (by
            change ChannelConfigRel D₀ j₀ realize
                {s with
                ...
                _
            exact hrelArg)
      exact stacked_recLam_app_presentedChannelConfigCompleteness D₀ j₀
        realize (s := s) (self := self) (x := x) (body := body)
        (arg := arg) hc hrel hargComplete
...
theorem funAppFrag_under_fun_frames_presentedChannelConfigCompleteness
    ...
    (hrel : ChannelConfigRel D₀ j₀ realize s answer) :
    PresentedChannelConfigCompleteness D₀ j₀ realize s answer := by
  have hVal' : ValueUnderFunFrameSpine D₀ j₀ realize (cloFrames frames) [] := by
    intro s' arg' answer' hc' hs' hscoped' hrel'
    have hs'' : s'.stack = functionStack frames := by
      simpa [funFrameStack_cloFrames, argumentStack] using hs'
    exact hVal hc' hs'' hscoped' hrel'
  exact funAppFrag_under_fun_frame_spine_of_value D₀ j₀ realize hfrag
    (cloFrames frames) [] (fun _ => rfl) hVal' hc
    (by simpa [funFrameStack_cloFrames, argumentStack] using hs)
    hscoped hrel
...
theorem funAppFrag_under_funs_then_args_of_value
    ...
    (hrel : ChannelConfigRel D₀ j₀ realize s answer) :
    PresentedChannelConfigCompleteness D₀ j₀ realize s answer := by
  have hVal' : ValueUnderFunFrameSpine D₀ j₀ realize (cloFrames fns) args := by
    intro s' arg' answer' hc' hs' hscoped' hrel'
    have hs'' : s'.stack = mixedStack fns args := by
      simpa [funFrameStack_cloFrames_append_args] using hs'
    exact hVal hc' hs'' hscoped' hrel'
  have hempty : cloFrames fns = [] → args = [] := by
    intro h
    cases fns with
    | nil => exact False.elim (hne rfl)
    | cons _ _ => cases h
  exact funAppFrag_under_fun_frame_spine_of_value D₀ j₀ realize hfrag
    (cloFrames fns) args hempty hVal' hc
    (by simpa [funFrameStack_cloFrames_append_args] using hs)
    hscoped hrel
...
theorem funAppFrag_under_funs_then_rec_arg_of_value
    ...
    (hrel : ChannelConfigRel D₀ j₀ realize s answer) :
    PresentedChannelConfigCompleteness D₀ j₀ realize s answer := by
  have hVal' : ValueUnderFunFrameSpine D₀ j₀ realize
      (cloFrames fns ++ [.recClo self x body cloEnv]) args := by
    intro s' arg' answer' hc' hs' hscoped' hrel'
    have hs'' : s'.stack =
        functionStack fns ++
          (.function (.recClosure self x body cloEnv) ::
            argumentStack args) := by
      simpa [funFrameStack_cloFrames_recClo_append_args] using hs'
    exact hVal hc' hs'' hscoped' hrel'
  exact funAppFrag_under_fun_frame_spine_of_value D₀ j₀ realize hfrag
    (cloFrames fns ++ [.recClo self x body cloEnv]) args
    (fun h => by
      have : False := by
        simpa [cloFrames] using h
      exact this.elim)
    hVal' hc
    (by simpa [funFrameStack_cloFrames_recClo_append_args] using hs)
    hscoped hrel
