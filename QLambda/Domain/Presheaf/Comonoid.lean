...
`A = 1` cofree UP `bangCofreeEquiv_one` / `comonoidHomEquiv_one` /
`bangCofreeEquiv_of_le_one` (comult glue closed); **co-Kleisli / bang comonad
interface for `A ≤ 1`** (`bangPromote`, `bangMap`, UP laws).
