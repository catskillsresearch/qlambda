...
This file bundles the published quantum-CPO objects and Scott-continuous
quantum functions.  The order on function homs is the pointwise order from
Kornell--Lindenhovius--Mislove: `F ⊑ G` iff `G ≤ S ∘ F`.

Unlike `qRelCategory`, these are functions rather than arbitrary relations.
They are the morphisms of the intended higher-order linear category.
-/
