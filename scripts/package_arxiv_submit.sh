...
mapfile -t LEAN_FILES < <(
  printf '%s\n' QLambda.lean Challenge.lean Solution.lean
  find QLambda -name '*.lean' | sort
)
...
lean = (
    ["QLambda.lean", "Challenge.lean", "Solution.lean"]
    + sorted(p.as_posix() for p in Path("QLambda").rglob("*.lean"))
)
for name in lean:
...
if [[ ${#FIGURE_PNGS[@]} -gt 0 ]]; then
  zip_args+=("${FIGURE_PNGS[@]}")
fi
zip -r "$ZIP" "${zip_args[@]}"
